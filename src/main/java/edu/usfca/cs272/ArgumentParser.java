...
public class ArgumentParser {
	...
	public static boolean isFlag(String arg) {
		// Check if argument is null to avoid error
		return arg != null && !arg.isEmpty() && arg.startsWith("-") && arg.length() > 1
	...
	public final void parse(String[] args) {
		for (int i = 0; i < args.length; i++) {
			...
			if (isFlag(argument)) {
				String value = null;
				if (i + 1 < args.length) {
					String argAfterFlag = args[i + 1];
					// Checks if argument after a flag is a value
					if (isValue(argAfterFlag)) {
						value = argAfterFlag;
						i++;
					}
				}
