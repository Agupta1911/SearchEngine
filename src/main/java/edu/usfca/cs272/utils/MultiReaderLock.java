...
public class MultiReaderLock {
	...
	private class ReadLock implements SimpleLock {
		...
		public void unlock() throws IllegalStateException {
			synchronized (lock) {
				if (readers <= 0) {
					throw new IllegalStateException("IllegalStateException: There are no readers to unlock");
				}
				readers--;
	...
	private class WriteLock implements SimpleLock {
		...
		public void unlock() throws IllegalStateException, ConcurrentModificationException {
			synchronized (lock) {
				if (writers <= 0) {
					throw new IllegalStateException("IllegalStateException: There are no writers to unlock");
				}
				if (!isActiveWriter()) {
					throw new ConcurrentModificationException(
							"ConcurrentModificationException: The thread does not hold the write lock");
				}
